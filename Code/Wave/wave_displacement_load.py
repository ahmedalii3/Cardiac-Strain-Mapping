...
class Wave_Displacer:
    ...
    def plot(self):
        ...
        def update(frame):
            ...
            Zx_disp = self.saved_x_dsip[f'arr_{self.frame_count}']
            Zy_dsip = self.saved_y_dsip[f'arr_{self.frame_count}']
            self.frame_count += 1

            # print(f"Max Value in Zx before strain validation: {np.max(Zx_disp)}")
            # result = limit_strain_range(Zx_disp, Zy_dsip, strain_lower_bound=0, strain_upper_bound=0.4)
