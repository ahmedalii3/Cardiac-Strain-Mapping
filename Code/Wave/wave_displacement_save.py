...
class Wave_Displacer:
    def __init__(self, path, save_mode=False):
        ...
        self.x_disp = []
        self.y_disp = []
        self.saved_x_dsip = np.load('displacments/displacments_x.npz')
        self.saved_y_dsip = np.load('displacments/displacments_y.npz')
    ...
    def save_stack(self):
        output_dir = "displacments"
        os.makedirs(output_dir, exist_ok=True)
        np.savez_compressed("displacments/displacments_x.npz", *self.x_disp)
        np.savez_compressed("displacments/displacments_y.npz", *self.y_disp)
    def plot(self):
        ...
        def update(frame):
            ...
            # Apply the mask to the displacements 
            # Zx_disp = Zx_disp * displaced_mask
            # Zy_dsip = Zy_dsip * displaced_mask
            # print(f"Max Value in Zx after mask: {np.max(Zx_disp)}")
            ...
            self.x_disp.append(Zx_disp)
            self.y_disp.append(Zy_dsip)
