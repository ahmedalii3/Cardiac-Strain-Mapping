...
class Residual_Unet_1D_5K(tf.keras.Model):
    def __init__(self):
        super(Residual_Unet_1D_5K, self).__init__()

        # Encoder
        self.conv1 = tf.keras.layers.Conv2D(64, 5, padding='same',strides = 1)
        self.bn1 = tf.keras.layers.BatchNormalization()
        self.act1 = tf.keras.layers.Activation('relu')
        self.conv2 = tf.keras.layers.Conv2D(64, 5, padding='same', strides = 1)
        self.shortcut1 = tf.keras.layers.Conv2D(64, 1, padding='same',  strides = 1)
        self.bn2 = tf.keras.layers.BatchNormalization()
        self.add1 = tf.keras.layers.Add()

        self.idblock1 = Identity_block(128, 5, 2)
        self.idblock2 = Identity_block(256, 5, 2)
        self.idblock3 = Identity_block(512, 5, 2)

        # bridge
        self.bridge = Identity_block(1024, 5, 2)

        # Decoder
        # self.up_concat1 = Upsample_Concatenate_block()
        self.Upconv1 = Upconv_block(512,5)
        self.up_concat1 = tf.keras.layers.Concatenate()
        self.idblock4 = Identity_block(512, 5, 1)

        # self.up_concat2 = Upsample_Concatenate_block()
        self.Upconv2 = Upconv_block(256,5)
        self.up_concat2 = tf.keras.layers.Concatenate()
        self.idblock5 = Identity_block(256, 5, 1)

        # self.up_concat3 = Upsample_Concatenate_block()
        self.Upconv3 = Upconv_block(128,5)
        self.up_concat3 = tf.keras.layers.Concatenate()
        self.idblock6 = Identity_block(128, 5, 1)

        # self.up_concat4 = Upsample_Concatenate_block()
        self.Upconv4 = Upconv_block(64,5)
        self.up_concat4 = tf.keras.layers.Concatenate()
        self.idblock7 = Identity_block(64, 5, 1)
    ...
    def call(self, inputs):
        ...
        output_def = self.output_con_def(dense1)
