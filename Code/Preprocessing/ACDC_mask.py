...
for root, dirs, files in os.walk(base_dir):
    for dir_name in dirs:
        # Check if the directory name matches the ground truth folder pattern
        if '_gt.nii' in dir_name:
            gt_folder_path = os.path.join(root, dir_name)
            print(f"Entering ground truth folder: {gt_folder_path}")
            ...
            for file in os.listdir(gt_folder_path):
                if file.endswith('.nii') or file.endswith('.nii.gz'):
                    ...
                    new_file_path = os.path.join(gt_folder_path, file.replace('.nii.gz', '_modified.nii.gz').replace('.nii', '_modified.nii'))
