...
from Code.Wave_SimulatorV2.helper.helper import dilate_mask
...
def animate_deformed_masked_mri(Image, MaskHT0, FrameDisplX, FrameDisplY, output_filename="deformed_mri.mp4", save_file=False):
    """
    Creates an animated visualization of the deformed MRI image over time and saves it as an MP4.
    ...
    # Update function for animation
    def update(frame):
        nonlocal FrameDisplX , FrameDisplY, MaskHT0, MaskHT0_deformed
        # Compute displacement (negate to match MATLAB)
        T3DDispX = -FrameDisplX[:, :, frame].astype(np.float64)
        T3DDispY = -FrameDisplY[:, :, frame].astype(np.float64)
