...
class StrainWaveParams:
    ...
    """
    meshsize: int = 256  # Grid size (NxN)
...
def compute_displacement_frames(param_x, param_y, num_frames, max_displ_x, max_displ_y, apply_smoothing=True, interaction_factor=0.25):
    ...
    if apply_smoothing:
        displacement_x = gaussian_filter(displacement_x, sigma=(5, 5, 0))
        displacement_y = gaussian_filter(displacement_y, sigma=(5, 5, 0))
...
def run_wave_simulation(num_frames=30):
    ...
    interaction_factor = np.random.uniform(0.1, 0.5)
    # Compute wave displacement frames for X and Y directions
    #FrameDisplX = compute_displacement_frames(params_x, num_frames, max_displ_x)
    #FrameDisplY = compute_displacement_frames(params_y, num_frames, max_displ_y)
...
def animate_strain_histograms(Ep1All, Ep2All, Ep3All, output_filename="strain_histogram_animation.mp4", num_bins=50, bin_range=0.3):
    """
...
def animate_wave_displacement(FrameDisplX, FrameDisplY, wind_dir_x, wind_dir_y, wind_speed_x, wind_speed_y, output_filename="wave_simulation.mp4"):
    """
...
def animate_strain_maps(Ep1All, Ep2All, Ep3All, vmin=-0.15, vmax=0.15, output_filename="strain_animation_with_colorbars.mp4"):
    """
...
def animate_deformed_mri(MaskHT0, FrameDisplX, FrameDisplY, output_filename="deformed_mri.mp4"):
    """
    ...
    num_frames = FrameDisplX.shape[2]  # Number of frames in the animation
    height, width = MaskHT0.shape  # Image dimensions
    ...
    def update(frame):
        ...
        T3DDispX = -FrameDisplX[:, :, frame].astype(np.float32)
        T3DDispY = -FrameDisplY[:, :, frame].astype(np.float32)
